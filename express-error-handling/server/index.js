...
app.post('/api/grades', (req, res, next) => {
  ...
  if (!Number.isInteger(score) || score < 0 || score > 100) {
    throw new ClientError(400, 'score must be a positive less than 100');
  }
  if (!name || !course) {
  ...
  db.query(sql, params)
    .then(result => {
      ...
      if (!grade) {
        ...
        throw new ClientError(404, `cannot finde grade with gradeId ${gradeId}`);
      } else {
        res.json(grade);
      }
    })
...
app.put('/api/grades/:gradeId', (req, res, next) => {
  const gradeId = Number(req.params.gradeId);
  if (!Number.isInteger(gradeId) || gradeId < 1) {
    throw new ClientError(400, 'grade must be a integer');
  }
  const { name, course } = req.body;
  const score = Number(req.body.score);
  if (!Number.isInteger(score) || score < 0 || score > 100) {
